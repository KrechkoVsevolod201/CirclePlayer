...
- **Колесо управления (Click Wheel)** — верхняя кнопка Menu, нижняя Play/Pause, центр подтверждает выбор; крупные кнопки дают виброотклик, прокрутка обрабатывается по одному шагу
...
- **Темы оформления** — готовые классическая, хакерская, киберпанк и жёлто-чёрная темы; выбор цвета HSV-палитрой, сохранение и обмен пресетами в JSON
...
- [v1.6 — плавная навигация, виброотклик и палитра цветов](docs/releases/v1.6.md)
...
Перед загрузкой релиза настройте собственный ключ через `keystore.properties`
(шаблон: `keystore.properties.example`) или переменные `RELEASE_*`. Без ключа
Gradle создаёт неподписанные файлы; не публикуйте сборку с debug-подписью.
...
- Локализованные описания для магазинов: `fastlane/metadata/android/`.
